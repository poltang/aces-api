...
CREATE VIEW accounts AS SELECT
    ...
    (SELECT COUNT(*) FROM members WHERE id=m.id) as mcount,
    users.username, users.email, users.fullname,
    tenants.orgName, tenants.expiryDate
FROM members m
...
--
DROP VIEW IF EXISTS active_accounts;
CREATE VIEW active_accounts AS SELECT
    m.id, m.tenantId, m.role, m.status, m.isDefault,
    (SELECT COUNT(*) FROM members WHERE status='active' AND id=m.id) as mcount,
    users.username, users.email, users.fullname,
    tenants.orgName, tenants.expiryDate
FROM members m
LEFT JOIN users ON m.id=users.id
...
--
DROP VIEW IF EXISTS used_modules;
CREATE VIEW used_modules AS SELECT
...
INSERT INTO admins ([id],[username],[fullname],[email],[role],[status]) VALUES
('6397c47ba009344a26c0db97','poltang','Gaia Poltangan','adminits@gaiasol.com','super-admin','active');